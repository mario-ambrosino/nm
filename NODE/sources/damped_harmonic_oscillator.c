...
void energy(double omega, int time, particle ho);
double dyn_funct(double omega, double y, double beta, double v);
void harmonic_oscillator_dynamic_step(particle ho, int step, float dt, float omega, float beta, int method);
...
int main(int argc, char *argv[]) {
  ...
  double omega = atof(argv[6]);                   // frequenza dell'oscillatore
	double beta = atof(argv[5]);                   // frequenza dell'oscillatore
  ...
  for(step = 0 ; step < epoch - 1 ; step++) {
    harmonic_oscillator_dynamic_step(ho, step, dt, omega, beta, method);
  }
...
void energy(double omega, int step, particle ho) {
  // valutiamo 2E / m
  ho.en[step] = ho.vl[step]*ho.vl[step] + omega*ho.ps[step]*ho.ps[step];
}

double dyn_funct(double omega, double y, double beta, double v) {
  return (- omega * y - beta * v);
}
...
void harmonic_oscillator_dynamic_step(particle ho, int step, float dt, float omega, float beta, int method) {
  ...
  if (method == 1) {
    // metodo di Eulero
    ho.vl[step+1] = ho.vl[step] + dt * dyn_funct(omega, ho.ps[step], beta, ho.vl[step]);
    ho.ps[step+1] = ho.ps[step] + dt * ho.vl[step];
    energy(omega, step, ho);
  ...
  if (method == 3) {
    ...
    for (i = 0; i < 3; i++) {
      ho.vl[step+1] = ho.vl[step] + dt * 0.5 * ( dyn_funct(omega, ho.ps[step+1], beta,
                       ho.vl[step+1]) + dyn_funct(omega, ho.ps[step], beta,ho.vl[step]));
  ...
  if (method == 5) {
    ...
    double C1_vl = dt * dyn_funct(omega, ho.ps[step], beta, ho.vl[step]);

    ho.ps[step+1] = ho.ps[step] + (ho.vl[step] + 0.5 * C1_vl) * dt;
    ho.vl[step+1] = ho.vl[step] + dyn_funct(omega, ho.ps[step] + 0.5 * C1_ps, beta, ho.vl[step] + 0.5 * C1_vl) * dt;
  ...
  if (method == 6) {
    ...
    double k1_ps = ho.vl[step];
		double k1_vl = dyn_funct(omega, ho.ps[step], beta, ho.vl[step]);

		double k2_ps = (ho.vl[step] + dt * 0.5 * k1_vl);
		double k2_vl = dyn_funct(omega, (ho.ps[step] + 0.5 * dt * k1_ps), beta, k2_ps);

		double k3_ps = (ho.vl[step] + dt * 0.5 * k2_vl);
		double k3_vl = dyn_funct(omega, (ho.ps[step] + 0.5 * dt * k2_ps), beta,  k3_ps);

		double k4_ps = (ho.vl[step] + dt * k3_vl);
    double k4_vl = dyn_funct(omega, (ho.ps[step] + dt * k3_ps), beta, k4_ps);
