...
//
typedef struct data_struct particle;
int ID_counter = 1;
...
data new_4D_data(int New_Dimension, data Input) {
	Input.ID = ID_counter++;
	Input.Dim = New_Dimension;
//	printf("\ndata[%i] defined. Size = %lu Bytes.", Input.ID, Input.Dim * sizeof(double) * 2);
	Input.t = (double *) calloc(Input.Dim, sizeof(double));
if (Input.t == NULL) {
//		printf("\nFailure in dynamical memory allocation of the X array.");
	exit(EXIT_FAILURE);
	}
...
void Export_4D_data(data Input, char name_file[255]) {
	int i;
	FILE *input_file;
	if ((input_file = fopen(name_file,"w")) == NULL ) {
		printf("\nFailure in file opening. \nClosing...");
	...
	for(i = 0 ; i < Input.Dim; i++) {
		fprintf(input_file, "%lf %lf %lf %lf\n", Input.t[i], Input.x[i], Input.y[i], Input.z[i]);
		}
	fclose(input_file);
//	printf("\nExport Completed.\n");
}
